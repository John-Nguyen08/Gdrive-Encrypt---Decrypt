...
//For Encrpytion: ./cryptomaniac <input_file> <output_file> -mctr -e -k <32bytes key> -i <16 bytes>
//For Decryption: ./cryptomaniac <input_file> <output_file> -mctr -d -k <32bytes key> -i <16 bytes>
	...
	for(int i = 3; i < argc; i++)
	{
		if(!strcmp(argv[i], "-e")) {
			...
			if(i < argc - 1) {
				...
				int st = hex2bin(argv[i + 1], cm->key, EVP_MAX_KEY_LENGTH); //NEED TO CHANGE THIS TO PSUEDORANDOM GENERATION
				if(st <= 0)
					goto cleanup;
				has_key = 1;
				i++;
			...
			if(i < argc - 1) {
				...
				int st = hex2bin(argv[i + 1], cm->iv, EVP_MAX_IV_LENGTH); //NEED TO CHANGE TO PSEUDORANDOM GENERATION
				if(st <= 0)
					goto cleanup;
				has_iv = 1;
				i++;
